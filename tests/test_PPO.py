...
def test_can_train_with_different_environment_dimensions(
    ...
    _train_agent_and_store_result(
        agent=PPO, config=config, environment=cartpole_environment
    )
    _assert_n_rows_where_stored(
    ...
    _train_agent_and_store_result(
        agent=PPO, config=config, environment=mountain_car_environment
    )
    _assert_n_rows_where_stored(
...
def test_can_train_for_continuous_environments(
    ...
    _train_agent_and_store_result(
        agent=PPO, config=config, environment=continuous_mountain_car_environment
    )
    _assert_n_rows_where_stored(
...
def test_can_train_for_atari_environments(
    ...
    _train_agent_and_store_result(
        agent=PPO, config=config, environment=adventure_environment
    )
    _assert_n_rows_where_stored(
