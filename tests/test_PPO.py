...
MOUNTAIN_CAR_CONTINUOUS_TEST_RESULTS = "mountain_car_continuous_ppo_test"
...
def test_can_train_with_different_environment_dimensions(
    ...
    _train_agent_and_store_result(agent=PPO, config=config, environment=cartpole_environment)
    _assert_n_rows_where_stored(
        filepath=f"{PATH_TO_TEST_RESULTS}{CARTPOLE_TEST_RESULTS}.csv", n=3
    )

    config = mountain_car_config(MOUNTAIN_CAR_TEST_RESULTS)
    _train_agent_and_store_result(agent=PPO, config=config, environment=mountain_car_environment)
    _assert_n_rows_where_stored(
...
def test_can_train_for_continuous_environments(
    ...
    _train_agent_and_store_result(agent=PPO, config=config, environment=continuous_mountain_car_environment)
    _assert_n_rows_where_stored(
